...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>discord.gg/zxDnqEcw<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>さま┊Windy•Hub Beta<Color=/>"):Display()
    wait(.14)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Support All Executor<Color=/>"):Display()
    wait(.24)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Join Discord For See Update Log Hub<Color=/>"):Display()
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>discord.gg/zxDnqEcw <Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "さま┊Windy•Hub", HidePremium = false, IntroText = "さま┊Windy•Hub Library", SaveConfig = true, ConfigFolder = "さま┊Windy•Hub"})
...
W:AddLabel("[+]  Have Bug Please Report in Discord Server")
W:AddLabel("[+]  Fast Loading")
    ...
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                ...
                OrionLib:MakeNotification({
                    Name = "さま┊Windy•Hub(Noitification)",
                    Content = "Please Cancel Quest (Will Have)",
                    Image = "",
                    Time = 9
                })
            end
